...
class ezParallelPhase implements Serializable {
    ...
    parallelPhase(script, String name = "ez parallel", boolean failFast = false, boolean failOnError = true, boolean showStages = true, boolean showPhaseStage = false) {
    ...
    void addSubJob(Map config) {
        ...
        if (config.job == null) {
            script.ezLog.info ("[ERROR] you must provive a job name to run!!!")
            throw Exception
        }
    ...
    void addStepsSequence(Map config) {
        ...
        if (config.sequence == null) {
            script.ezLog.info ("[ERROR] you must provive a block of steps sequence to run!!!")
            throw Exception
        }
        if (config.name == null) {
    ...
    def runStepsSequence(def item) {
        ...
        def duration = TimeCategory.minus(timeStop, timeStart)
        item.duration = duration.toString()
        script.ezLog.info(" '\033[1;94m${item.title}\033[0m' ended with \033[1;94m${item.status}\033[0m status. Duration: \033[1;94m${duration}\033[0m")
    }
